...
def main(params=[]):
    ...
    parser.add_argument('--group_by', type=str, default="stimuli_file", help='Column by which the data is being grouped and plotted')

    argsdict = vars(parser.parse_args())
    lInfo("read {}".format(argsdict["cvsfilename"]))

    # Check if file exists
    if not os.path.exists(argsdict["cvsfilename"]):
        lError("File not found: {}".format(argsdict["cvsfilename"]))
        sys.exit(1)
